...
const valid = (): boolean => {
    ...
    const isEmpty = currentOptions.value.length === 0;
    const lessThanMinLimit = currentOptions.value.length < props.question.minOptionsLimit;
    const moreThanMaxLimit = currentOptions.value.length > props.question.maxOptionsLimit;
    ...
    if (isEmpty) {
        if (props.question.required && overTheLimits) {
           error.value = 'This question is required';
            ...
            error.value = `Answer must be at least ${props.question.minOptionsLimit} options selected`;
    } else if (moreThanMaxLimit) {
            error.value = `Answer must be at most ${props.question.maxOptionsLimit} options selected`;
    }
